...
int32 udp_t::receive(net_buf_t* buf, uint32 ip)
{
    ...
    buffer->append(&pseudo_hdr, sizeof(udp_pseudo_hdr_t));
    buffer->append(buf->get_data(), buf->get_data_len());
    ...
    /* FIXME: only test dns resolve, should change to check sock_dgram after implemented */
    if (net_t::ntohs(hdr->m_src_port) == 53) {
        buf->pop_front(sizeof(udp_hdr_t));
        return dns_t::receive(buf);
    }
